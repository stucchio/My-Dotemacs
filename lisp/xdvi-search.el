...
;;;
;;; Copyright (C) 2004, 2005 Stefan Ulrich
;;; Copyright (C) 2006, Chris Stucchio (minor modifications)
;;; $Revision: 1.35 $, tested with Emacs 20.4 to 21.2 and Xemacs 21.1 to 21.5
;;;
  ...
  (save-excursion
    (save-restriction
      ...
      (let* (;;; Current line in file.
	     ...
	     ;;; DVI file name:
	     (dvi-file (get-source-filename (concat (file-name-sans-extension master-file) ".dvi") flag))
...
(defun xdvi-works ()
  "Returns t if xdvi inverse search will work, nil if not."
  (condition-case nil
      (xdvi-has-inverse-search (car (xdvi-get-version-number xdvi-version-number)))
    (error nil)
    )
  )
